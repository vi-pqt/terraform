...
variable "enable_mysql_ecs" {
  description = "Enable MySQL ECS task (disable when using Aurora — Session 09)"
  type        = bool
  default     = true
}

variable "enable_redis_ecs" {
  description = "Enable Redis ECS task (disable when using ElastiCache — Session 10)"
  type        = bool
  default     = true
}

# ── Aurora MySQL (Session 09 / 14) ──
...
variable "aurora_instance_class" {
  description = "Aurora instance class (lab: db.t3.medium)"
  type        = string
...
# ── ElastiCache Redis (Session 10 / 14) ──
variable "enable_elasticache" {
  description = "Enable ElastiCache Redis (disable ECS Redis first: enable_redis_ecs = false)"
  type        = bool
  default     = false
}

variable "elasticache_auth_token" {
  description = "ElastiCache Redis AUTH token (>= 16 chars). Encrypt with sops for Git."
  type        = string
  sensitive   = true
  default     = null
}
...
variable "elasticache_node_type" {
  description = "ElastiCache node type (lab: cache.t3.micro)"
  type        = string
...
# ── KMS (shared infra — read from remote state) ──
